...
trusted_sources = {'BBC': 9.5, 'Reuters': 9.2, 'The Guardian': 9, 'Associated Press': 9.3, 'Bloomberg': 9.1}
fake_keywords = ['hoax', 'exposed', 'fake', 'banned', 'scandal']
truth_keywords = ['report', 'confirmed', 'source', 'official', 'investigation']
...
while True:
    print(f"Starting fact-check update: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

   
    articles_resp = supabase.table("articles").select("*").is_("fact_check_score", None).execute()
    articles = articles_resp.data

    if not articles:
        print("No new articles to evaluate.")
    else:
        for article in articles:
            ...
            if not source_resp.data:
                print(f"No source info for article {article_id}, skipping.")
                continue
